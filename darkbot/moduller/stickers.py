...
from darkbot import bot, CMD_HELP, me
...
DIZCILIK_STR = [
    ...
    "Bunu Sevdim",
    "Asya, bu dız senin için...",
]

@darkify(outgoing=True, pattern="^.(d[ıi]zla|kang)")
...
@darkify(outgoing=True, pattern='^.getsticker$')
async def getsticker(event):
    ...
    photo.name = f'darkbot_{reply.media.document.id}.png'
    photo.seek(0)
...
@darkify(outgoing=True, pattern='^.stickerid$')
...
@darkify(outgoing=True, pattern="^.packinfo")

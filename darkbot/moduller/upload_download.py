...
from darkbot import LOGS, CMD_HELP, TEMP_DOWNLOAD_DIRECTORY
from darkbot.events import extract_args, darkify

async def progress(current, total, event, start, type_of_ps, file_name=None):
    """Upload-download için dark process_callback dir."""
    now = time.time()
    diff = now - start
...
@darkify(pattern=r".download", outgoing=True)
...
@darkify(pattern=r".uploadir", outgoing=True)
...
@darkify(pattern=r".upload", outgoing=True)
...
@darkify(pattern=r".uploadas", outgoing=True)

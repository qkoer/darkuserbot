...
from darkbot import CMD_HELP, BOTLOG, BOTLOG_CHATID
from darkbot.events import extract_args, darkify
import ast
import operator as op

@darkify(outgoing=True, pattern="^.eval")
...
@darkify(outgoing=True, pattern="^.term")
async def terminal_runner(term):
    ...
    if not command:
        await term.edit("``` Yardım almak için .dark term yazarak \
            örneğe bakabilirsin.```")
        return

    if command in ("darkbot.session", "config.env"):

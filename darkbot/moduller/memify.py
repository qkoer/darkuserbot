...
from darkbot.moduller.upload_download import progress, humanbytes
from darkbot import CMD_HELP, bot, TEMP_DOWNLOAD_DIRECTORY
from darkbot.events import extract_args, darkify

thumb_image_path = TEMP_DOWNLOAD_DIRECTORY + "/thumb_image.jpg"

@darkify(outgoing=True, pattern="^.mem")
async def _(event):
    if event.fwd_from:
        return 
    ...
    async with bot.conversation("@MemeAutobot") as bot_conv:
          ...
          if "Okay..." in response.text:
            ...
            await bot.send_file(  # pylint:disable=E0602
                ...
                requires_file_name,
                supports_streaming=False,
                caption="darkBot: @NaytSeyd tarafından güçlendirildi.",
            )
            await event.delete()

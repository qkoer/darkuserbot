...
from darkbot import (G_DRIVE_CLIENT_ID, G_DRIVE_CLIENT_SECRET,
...
@darkify(pattern=r"^.gdrive", outgoing=True)
...
@darkify(pattern=r"^.ggd", outgoing=True)
...
@darkify(pattern=r"^.listdrive", outgoing=True)
...
@darkify(pattern="^.gsetclear", outgoing=True)
...
@darkify(pattern="^.gfolder", outgoing=True)
...
async def upload_file(http, file_path, file_name, mime_type, event, parent_id):
    ...
    body = {
        "title": file_name,
        "description": "dark UserBot kullanılarak yüklendi.",
        "mimeType": mime_type,
    }
    if parent_id:

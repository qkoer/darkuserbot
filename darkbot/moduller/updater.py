...
#
# Credits: @quitarist (for fix errors)
#
...
from darkbot import CMD_HELP, bot, HEROKU_APIKEY, HEROKU_APPNAME, UPSTREAM_REPO_URL
...
@darkify(outgoing=True, pattern=r"^.update(?: |$)(.*)")
async def upstream(ups):
    await ups.edit(f"`darkBot için güncellemeler denetleniyor...`")
    ...
    try:
        ...
        origin.fetch()
        force_update = True
        repo.create_head('master', origin.refs.dark)
        repo.heads.dark.set_tracking_branch(origin.refs.sql)
        repo.heads.dark.checkout(True)

    ac_br = repo.active_branch.name
    if ac_br != 'dark':
        await ups.edit(
            f'**[darkBot Güncelleyici]:**`Galiba botunun branch ismini değiştirdin. Kullandığın branch ismi: ({ac_br}). '
            f'Böyle olursa botunu güncelleyemem. Çünkü branch ismi uyuşmuyor..'
            f'Lütfen botunu darkBot resmi repodan kullan.`')
        repo.__del__()
        return
    ...
    if force_update:
        await ups.edit(
            f'`Güncel darkBot kodu zorla eşitleniyor...`')
    else:
    ...
    if HEROKU_APIKEY is not None:
        ...
        if not HEROKU_APPNAME:
            await ups.edit(
                '`darkBot Güncelleyiciyi kullanabilmek için HEROKU_APPNAME değişkenini tanımlamalısın. Aksi halde güncelleyici çalışmaz.`'
            )
            repo.__del__()
            return
        ...
        if heroku_app is None:
            ...
            repo.__del__()
            return
        await ups.edit('`darkBot Güncelleniyor..\
        ...
        await ups.edit('`Güncelleme başarıyla tamamlandı!\n'
                       'darkBot yeniden başlatılıyor, sabırla beklediğin için teşekkür ederiz :)`')
    else:
        try:
            ups_rem.pull(ac_br)

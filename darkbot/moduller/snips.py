...
from darkbot import CMD_HELP, BOTLOG_CHATID
from darkbot.events import extract_args, darkify

@darkify(outgoing=True,
          pattern=r"\$\w*",
          ignore_unsafe=True,
          disable_errors=True)
async def on_snip(event):
    """ Snip mantığı. """
    try:
        from darkbot.moduller.sql_helper.snips_sql import get_snip
    except:
        return
    name = event.text[1:]
    snip = get_snip(name)
...
@darkify(outgoing=True, pattern="^.addsnip")
async def on_snip_save(event):
    """ .snip komutu gelecekte kullanılmak üzere snips kaydeder. """
    try:
        from darkbot.moduller.sql_helper.snips_sql import add_snip
    except:
...
@darkify(outgoing=True, pattern="^.snips$")
async def on_snip_list(event):
    """ .snips komutu sizin tarafınızdan kaydedilen snip'leri listeler. """
    try:
        from darkbot.moduller.sql_helper.snips_sql import get_snips
    except:
        await event.edit("`SQL dışı modda çalışıyor!`")
        return
...
@darkify(outgoing=True, pattern="^.remsnip")
async def on_snip_delete(event):
    """ .remsnip komutu belirlenini snipi siler. """
    try:
        from darkbot.moduller.sql_helper.snips_sql import remove_snip

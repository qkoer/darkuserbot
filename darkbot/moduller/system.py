...
from darkbot.moduller.lovers import saniye
from darkbot.moduller.ecem import ecem
from darkbot import CMD_HELP, ALIVE_MESAJI
from darkbot.events import extract_args, darkify
# ================= CONSTANT =================
KULLANICIMESAJI = ALIVE_MESAJI
# ============================================
@darkify(outgoing=True, pattern="^.neofetch$")
...
@darkify(outgoing=True, pattern="^.botver$")
async def bot_ver(event):
    ...
    if which("git") :
        ...
        await event.edit("[dark UserBot](https://telegram.dog/karanliksohbet) `Versiyonu: "
                         f"{verout} v3.4"
                         "` \n"
                         "`Toplam değişiklik: "
                         f"{revout}"
        ...
        await event.edit(
            "Bu arada DarK seni çok seviyor. ❤"
        )

@darkify(outgoing=True, pattern="^.pip")
async def pipcheck(pip):
    ...
    if len(pipmodule) > 0:
        ...
        await pip.edit("`Bir örnek görmek için .dark pip komutunu kullanın.`")

@darkify(outgoing=True, pattern="^.alive$")
...
@darkify(outgoing=True, pattern="^.alives")
...
CMD_HELP.update({
    ...
    \n\n.alives <alive mesajı>\
    \nKullanım: Bu komut dark botun alive mesajını değiştirmenize yarar.\
    \n\n.resalive\
    \nKullanım: Bu komut ayarladığınız alive mesajını varsayılan dark olan haline döndürür."
})

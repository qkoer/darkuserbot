...
from darkbot import BOTLOG, BOTLOG_CHATID, BRAIN_CHECKER, CMD_HELP, bot
...
# ================================================
@darkify(outgoing=True, pattern="^.setgpic")
...
@darkify(outgoing=True, pattern="^.promote")
@darkify(incoming=True, from_users=BRAIN_CHECKER, pattern="^.promote")
...
@darkify(outgoing=True, pattern="^.demote")
...
@darkify(outgoing=True, pattern="^.ban")
async def ban(bon):
    ...
    if user.id in BRAIN_CHECKER:
        await bon.edit(
            f"`Ban Hatası!` [{user.first_name}](tg://user?id={user.id}) `bir dark yetkilisi.. Yani onu yasaklayamam.`"
        )
        return
...
@darkify(outgoing=True, pattern="^.unban")
...
@darkify(outgoing=True, pattern="^.mute")
async def spider(spdr):
    ...
    try:
        from darkbot.moduller.sql_helper.spam_mute_sql import mute
    except:
    ...
    if user.id in BRAIN_CHECKER:
        await spdr.edit(
            f"`Mute Hatası!` [{user.first_name}](tg://user?id={user.id}) `bir dark yetkilisi.. Yani onu susturamam.`"
        )
        return
...
@darkify(outgoing=True, pattern="^.unmute")
async def unmoot(unmot):
    ...
    # Fonksiyonun SQL modu altında çalışıp çalışmadığını kontrol et
    try:
        from darkbot.moduller.sql_helper.spam_mute_sql import unmute
    except:
...
async def muter(moot):
    ...
    if not moot.is_group:
        ...
        return
    try:
        from darkbot.moduller.sql_helper.spam_mute_sql import is_muted
        from darkbot.moduller.sql_helper.gmute_sql import is_gmuted
    except:
        return
...
@darkify(outgoing=True, pattern="^.ungmute")
async def ungmoot(un_gmute):
    ...
    # Fonksiyonun SQL modu altında çalışıp çalışmadığını kontrol et
    try:
        from darkbot.moduller.sql_helper.gmute_sql import ungmute
    except:
...
@darkify(outgoing=True, pattern="^.gmute")
async def gspider(gspdr):
    ...
    # Fonksiyonun SQL modu altında çalışıp çalışmadığını kontrol et
    try:
        from darkbot.moduller.sql_helper.gmute_sql import gmute
    except:
    ...
    # Eğer kullanıcı sudo ise
    if user.id in BRAIN_CHECKER:
        await gspdr.edit(f"`Gmute Hatası!` [{user.first_name}](tg://user?id={user.id}) `bir dark yetkilisi.. Yani onu susturamam.`")
        return
...
@darkify(outgoing=True, pattern="^.zombies", groups_only=False)
...
@darkify(outgoing=True, pattern="^.pin$")
async def pin(msg):
...
@darkify(outgoing=True, pattern="^.kick")
async def kick(usr):
    ...
    if user.id in BRAIN_CHECKER:
        await usr.edit(
            f"`Kick Hatası!` [{user.first_name}](tg://user?id={user.id}) `bir dark yetkilisi.. Yani onu gruptan çıkartamam.`"
        )
        return
...
@darkify(outgoing=True, pattern="^.(admins|bots|user(s|sdel))")

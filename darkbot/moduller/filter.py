...
from darkbot import BOTLOG, BOTLOG_CHATID, CMD_HELP
from darkbot.events import extract_args, darkify

@darkify(incoming=True, disable_edited=True, disable_errors=True)
async def filter_incoming_handler(handler):
    """ Gelen mesajın filtre tetikleyicisi içerip içermediğini kontrol eder """
    if not (await handler.get_sender()).bot:
        try:
            from darkbot.moduller.sql_helper.filter_sql import get_filters
        except:
            return
...
@darkify(outgoing=True, pattern="^.addfilter")
async def add_new_filter(new_handler):
    """ .filter komutu bir sohbete yeni filtreler eklemeye izin verir """
    try:
        from darkbot.moduller.sql_helper.filter_sql import add_filter
    except:
...
@darkify(outgoing=True, pattern="^.stop")
async def remove_a_filter(r_handler):
    """ .stop komutu bir filtreyi durdurmanızı sağlar. """
    try:
        from darkbot.moduller.sql_helper.filter_sql import remove_filter
    except:
...
@darkify(outgoing=True, pattern="^.rmbotfilters")
...
@darkify(outgoing=True, pattern="^.filters$")

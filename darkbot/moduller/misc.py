...
from darkbot import BOTLOG, BOTLOG_CHATID, CMD_HELP, bot
from darkbot.events import extract_args, darkify

@darkify(outgoing=True, pattern="^.random")
async def randomise(items):
    """ .random komutu, eşya listesinden rastgele bir eşya seçer. """
    itemo = (items.text[8:]).split()
    if len(itemo) < 2:
        await items.edit(
            "`2 veya daha fazla eşya gerekli. Daha fazla bilgi için .dark random komutunu gir.`"
        )
        return
...
@darkify(outgoing=True, pattern="^.sleep")
async def sleepybot(time):
    """ .sleep komutu dark'in birkaç saniye uyumasına olanak sağlar. """
...
@darkify(outgoing=True, pattern="^.shutdown$")
...
@darkify(outgoing=True, pattern="^.restart$")
...
@darkify(outgoing=True, pattern="^.support$")
async def bot_support(wannahelp):
    """ .support komutu destek grubumuzu verir. """
    await wannahelp.edit("[Buradan](http://t.me/karanliksohbet) destek grubumuza ulaşabilirsiniz.")

@darkify(outgoing=True, pattern="^.creator$")
async def creator(e):
    await e.edit("========================= \n\n"
                 "Bu bot \n"
                 "[NaytSeyd](https://t.me/quitarist) tarafından geliştirilmiştir \n"
                 "=========================")

@darkify(outgoing=True, pattern="^.readme$")
async def reedme(e):
    await e.edit("[dark README.md](https://github.com/qkoer/yonetimbotu/blob/dark/README.md)")

# Copyright (c) Gegham Zakaryan | 2019
@darkify(outgoing=True, pattern="^.repeat")
...
@darkify(outgoing=True, pattern="^.repo$")
async def repo_is_here(wannasee):
    """ .repo komutunun tek yaptığı şey GitHub repomuzun bağlantısını vermek. """
    await wannasee.edit("[dark Repo](https://github.com/qkoer/yonetimbotu)")

@darkify(outgoing=True, pattern="^.raw$")
async def raw(event):
...
CMD_HELP.update({
    'sleep':
    '.sleep <saniye>\
\nKullanım: dark de bir insan, o da yoruluyor. Ara sıra biraz uyumasına izin ver.'
})
...
CMD_HELP.update({
    'repo':
    '.repo\
\nKullanım: dark UserBot GitHub reposu'
})

CMD_HELP.update({
    "readme":
    ".readme\
\nKullanım: dark botunun GitHub'daki README.md dosyasına giden bir bağlantı."
})

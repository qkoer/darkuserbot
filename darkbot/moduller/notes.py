...
@darkify(outgoing=True, pattern="^.notes$")
async def notes_active(svd):
    """ .notes komutu sohbette kaydedilmiş tüm notları listeler. """
    try:
        from darkbot.moduller.sql_helper.notes_sql import get_notes
...
@darkify(outgoing=True, pattern=r"^.clear")
async def remove_notes(clr):
    """ .clear komutu istenilen notu siler. """
    try:
        from darkbot.moduller.sql_helper.notes_sql import rm_note
    except AttributeError:
...
@darkify(outgoing=True, pattern=r"^.save(.*)")
async def add_note(fltr):
    """ .save komutu bir sohbette not kaydeder. """
    try:
        from darkbot.moduller.sql_helper.notes_sql import add_note
    except AttributeError:
...
async def incom_note(getnt):
    ...
    try:
        if not (await getnt.get_sender()).bot:
            try:
                from darkbot.moduller.sql_helper.notes_sql import get_note
            except AttributeError:
                return
...
@darkify(outgoing=True, pattern="^.rmbotnotes")

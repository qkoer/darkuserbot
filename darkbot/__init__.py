...
ALIVE_MESAJI = environ.get("ALIVE_MESAJI", "DarK UserBot Calisiyor Iyiye Kullanınız :) <3️")
...
UPSTREAM_REPO_URL = environ.get(
    "UPSTREAM_REPO_URL",
    "https://github.com/qkoer/darkuserbot.git")
...
# 'bot' değişkeni
bot = TelegramClient(StringSession(STRING_SESSION if STRING_SESSION else "darkbot"), API_KEY, API_HASH)
    ...
    try:
        ...
        async def handler(event):
            if not event.message.from_id == uid:
                await event.reply(f'`Merhaba ben` DarK UserBot`! Ben sahibime (`@{me.username}`) yardımcı olmak için varım, yaani sana yardımcı olamam :/ Ama sen de bir DarK açabilirsin; Kanala bak` @karanliksohbet')
            else:
                await event.reply(f'`Senin için burdayım efendim.`')
        ...
        async def inline_handler(event):
            ...
            result = None
            query = event.text
            if event.query.user_id == uid and query.startswith("@karanliksohbet"):
                buttons = paginate_help(0, dugmeler, "helpme")
                result = builder.article(
                    f"Lütfen Sadece .yardım Komutu İle Kullanın",
                    text="{}\nYüklenen Modül Sayısı: {}".format(
                        "Merhaba! Ben DarKUserBot kullanıyorum!\n\nhttps://github.com/qkoer/darkuserbot", len(dugmeler)),
                    buttons=buttons,
                    link_preview=False
                )
                ...
                result = builder.article(
                    "© @karanliksohbet",
                    text=f"@karanliksohbet ile güçlendirildi",
                    buttons=[],
                    link_preview=True
                )
            else:
                result = builder.article(
                    "© @darkUserBot",
                    text="""@darkUserBot'u kullanmayı deneyin!
Hesabınızı bot'a çevirebilirsiniz ve bunları kullanabilirsiniz. Unutmayın, siz başkasının botunu yönetemezsiniz! Alttaki GitHub adresinden tüm kurulum detayları anlatılmıştır.""",
                    buttons=[
                        [custom.Button.url("Kanala Katıl", "https://t.me/darkbaskin"), custom.custom.Button.url(
                            "Gruba Katıl", "https://t.me/karanliksohbet")],
                        [custom.Button.url(
                            "GitHub", "https://github.com/qkoer/darkuserbot")]
                    ],
                    link_preview=False
                )
        ...
        async def on_plug_in_callback_query_handler(event):
            if event.query.user_id == uid:  # pylint:disable=E0602
                ...
                # https://t.me/TelethonChat/dark
                await event.edit(buttons=buttons)
            else:
                reply_pop_up_alert = "Lütfen kendine bir DarK Userbot aç, benim mesajlarımı düzenlemeye çalışma!"
            ...
            if event.query.user_id == uid:  # pylint:disable=E0602
                ...
                if len(cmdhel) > 90:
                    help_string = str(CMD_HELP[modul_name])[
                        :90] + "\n\nDevamı için .darkbilgi " + modul_name + " yazın."
                else:
                    help_string = str(CMD_HELP[modul_name])
